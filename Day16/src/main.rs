...
fn update_state(initial: &State, to_str: String, to_idx: NodeIndex, t_spent: i32, pressure: i32) -> State{
    ...
    State::new(to_str, initial.idx, t, unopened, opened)
}
...
fn main() {
    ...
    let mut states: Vec<State> = vec![State::new(start.clone(), v[&start], 30, non_zero.clone(), vec![])];
    let mut end: Vec<Vec<(i32,i32)>> = vec![];
    let mut finished: Vec<Vec<(i32,i32)>> = vec![]; 
    let mut max_p = 0;
    while states.len() > 0{
        (states, finished) = iterate_states(&g,&v,states);
